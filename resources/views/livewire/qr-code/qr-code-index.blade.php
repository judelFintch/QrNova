    ...
    <div wire:loading.class="opacity-50" class="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        @forelse ($qrCodes as $qrCode)
            <article class="group overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
                ...
                <div class="p-5"><h2 class="truncate text-lg font-black text-slate-950">{{ $qrCode->name ?: 'Sans nom' }}</h2><p class="mt-2 line-clamp-2 min-h-10 break-all text-sm leading-5 text-slate-500">{{ $qrCode->content }}</p><div class="mt-5 grid grid-cols-2 gap-2"><a href="{{ route('qr-code.show', $qrCode) }}" class="rounded-xl bg-slate-950 px-4 py-2.5 text-center text-sm font-bold text-white transition hover:bg-indigo-600">Ouvrir</a><a href="{{ route('qr-code.edit', $qrCode) }}" class="rounded-xl border border-indigo-200 px-4 py-2.5 text-center text-sm font-bold text-indigo-600 transition hover:bg-indigo-50">Modifier</a><button type="button" wire:click="delete({{ $qrCode->id }})" wire:confirm="Supprimer définitivement ce QR Code ?" class="col-span-2 rounded-xl border border-red-200 px-4 py-2.5 text-sm font-bold text-red-600 transition hover:bg-red-50">Supprimer</button></div></div>
            </article>
        @empty
