<div class="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
    ...
    <div class="mt-6 grid items-start gap-8 lg:grid-cols-[380px_1fr]">
        ...
        <section class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl shadow-slate-200/50">
            <div class="border-b border-slate-200 p-6 sm:p-8"><span class="rounded-full bg-indigo-100 px-3 py-1 text-xs font-bold uppercase text-indigo-700">{{ \App\Services\QrCodeService::TYPES[$qrCode->type] ?? $qrCode->type }}</span><h1 class="mt-4 text-3xl font-black tracking-tight">{{ $qrCode->name ?: 'Sans nom' }}</h1><p class="mt-2 text-sm text-slate-500">Créé le {{ $qrCode->created_at->format('d/m/Y à H:i') }}</p></div>
